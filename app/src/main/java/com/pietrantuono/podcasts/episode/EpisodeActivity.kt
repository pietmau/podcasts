...
class EpisodeActivity : DetailActivtyBase(), SingleSubscribedPodcastView {
    companion object {
    ...
    private fun inject() {
        (application as App).applicationComponent?.with(EpisodeModule(this@EpisodeActivity))?.inject(this@EpisodeActivity)
    }

    private fun initViews() {
        setContentView(R.layout.episode_activity)
        ButterKnife.bind(this@EpisodeActivity)
        setUpActionBar()
