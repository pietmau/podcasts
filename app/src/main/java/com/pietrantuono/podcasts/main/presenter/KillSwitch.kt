...
class KillSwitch(
    ...
    fun checkIfNeedsToBeKilled(view: MainView?) {
        ...
        compositeSubscription.add(RxFirebaseDatabase
                .observeSingleValueEvent(reference.child(SUPPORTED_VERSIONS), { it?.value as? ArrayList<Int> })
                .flatMapIterable { it }
        ...
        compositeSubscription.add(RxFirebaseDatabase
                .observeSingleValueEvent(reference.child(APP_IS_SUPPORTED), { it?.value as? Boolean })
                .filter { it == false }
                .observeOn(io)
                .subscribeOn(mainThread)
                .subscribe(object : SimpleObserver<Boolean>() {
