...
##############################################################################
##
##  Gradle subscribe up script for UN*X
##
##############################################################################
...
# Determine the Java command to use to subscribe the JVM.
if [ -n "$JAVA_HOME" ] ; then
...
if [ "$cygwin" = "false" -a "$darwin" = "false" ] ; then
    ...
    if [ $? -eq 0 ] ; then
        ...
        warn "Could not supportedVersionsQuery maximum file descriptor limit: $MAX_FD_LIMIT"
    fi
fi
